...
Let `η` be a partition of `n` with at most `N` parts and let

`x_i = η_{N - 1 - i} + i`,  `i = 0, …, N - 1`

be its *first column hook lengths*, read from the bottom row upwards (`Young.frobVec`).  The
number `f^η` of standard Young tableaux of shape `η` is then given by the Frobenius
formula

`f^η · ∏_i x_i ! = n ! · ∏_{i < j} (x_j - x_i)`.
...
Vandermonde product: removing a removable corner from the row `N - 1 - i` of `η` decreases
`x_i` by one, and the shapes for which this is not allowed contribute zero because the
resulting family has a repeated entry.

## Main definitions

* `Young.frobVec N η` : the first column hook lengths of `η`, padded to `N` rows.
...
/-- The first column hook lengths of the partition `η`, padded to `N` rows and read from
the bottom row upwards: `frobVec N η i = η_{N - 1 - i} + i`.  These are `N` pairwise
distinct natural numbers which determine `η`. -/
def frobVec (N : ℕ) (η : List ℕ) (i : Fin N) : ℕ := η.getD (i.rev : ℕ) 0 + (i : ℕ)
...
lemma sum_frobVec {N : ℕ} {η : List ℕ} (hN : η.length ≤ N) :
    ∑ i : Fin N, frobVec N η i = η.sum + N.choose 2 := by
  have h1 : ∑ i : Fin N, frobVec N η i
      = (∑ i : Fin N, η.getD (i.rev : ℕ) 0) + ∑ i : Fin N, (i : ℕ) := by
    rw [← Finset.sum_add_distrib]
    rfl
  have h2 : ∑ i : Fin N, η.getD (i.rev : ℕ) 0 = η.sum :=
    calc ∑ i : Fin N, η.getD (i.rev : ℕ) 0 = ∑ i : Fin N, η.getD (i : ℕ) 0 :=
          Equiv.sum_comp (Fin.revPerm : Equiv.Perm (Fin N)) fun i : Fin N => η.getD (i : ℕ) 0
      _ = η.sum := by
          rw [sum_eq_sum_range η hN, ← Fin.sum_univ_eq_sum_range fun i => η.getD i 0]
...
variable {N : ℕ} {η : List ℕ} {r : Fin N}

/-- Removing a removable corner from the row `r` of `η` decreases the first column hook
length of index `r.rev` by one and leaves the others unchanged. -/
lemma frobVec_decrNth (hη : IsPart η) (hc : IsRemCorner η (r : ℕ)) :
    (fun p => ((frobVec N (decrNth η (r : ℕ)) p : ℕ) : ℤ))
      = Function.update (fun p => ((frobVec N η p : ℕ) : ℤ)) r.rev
          (((frobVec N η r.rev : ℕ) : ℤ) - 1) := by
  have hpos : 0 < η.getD (r : ℕ) 0 := by
  ...
  · rw [Function.update_of_ne hp]
    ...
    simp only [frobVec, getD_decrNth_of_ne hη hc hne]
...
lemma frobVec_prod_factorial_decrNth (hη : IsPart η) (hc : IsRemCorner η (r : ℕ)) :
    frobVec N η r.rev * ∏ p : Fin N, Nat.factorial (frobVec N (decrNth η (r : ℕ)) p)
      = ∏ p : Fin N, Nat.factorial (frobVec N η p) := by
  classical
  have hrev : ((r.rev : Fin N).rev : ℕ) = (r : ℕ) := by simp [Fin.rev_rev]
  have hpos : 0 < η.getD (r : ℕ) 0 := by
    rw [IsRemCorner] at hc; omega
  have hval : frobVec N (decrNth η (r : ℕ)) r.rev + 1 = frobVec N η r.rev := by
    simp only [frobVec, hrev, getD_decrNth_self]
    omega
  have hother : ∀ p : Fin N, p ≠ r.rev →
      frobVec N (decrNth η (r : ℕ)) p = frobVec N η p := by
    intro p hp
    have hne : (r : ℕ) ≠ (p.rev : ℕ) := by
      intro h
      apply hp
  ...
  rw [← Finset.mul_prod_erase _ _ (Finset.mem_univ r.rev),
    ← Finset.mul_prod_erase _ (fun p => Nat.factorial (frobVec N η p)) (Finset.mem_univ r.rev),
    ← mul_assoc]
  congr 1
...
/-- If the row `r` of `η` has no removable corner, then decreasing the corresponding first
column hook length by one produces a repetition, so the corresponding term of the shift
identity vanishes. -/
lemma frobVec_term_eq_zero_of_not_remCorner (hη : IsPart η) (hN : η.length ≤ N)
    (hc : ¬ IsRemCorner η (r : ℕ)) :
    ((frobVec N η r.rev : ℕ) : ℤ)
        * vdmProd (Function.update (fun p => ((frobVec N η p : ℕ) : ℤ)) r.rev
          (((frobVec N η r.rev : ℕ) : ℤ) - 1)) = 0 := by
  classical
  set i : Fin N := r.rev with hi
  have hirev : ((i.rev : Fin N) : ℕ) = (r : ℕ) := by simp [hi, Fin.rev_rev]
  have heq : η.getD ((r : ℕ) + 1) 0 = η.getD (r : ℕ) 0 := by
    have hle : η.getD ((r : ℕ) + 1) 0 ≤ η.getD (r : ℕ) 0 :=
      hη.getD_antitone (Nat.le_succ _)
    rw [IsRemCorner] at hc
    omega
  rcases Nat.eq_zero_or_pos (frobVec N η i) with hzero | hpos
  · rw [hzero]
    simp
  · have hi0 : 0 < (i : ℕ) := by
      by_contra h
      ...
      have hr : (r : ℕ) = N - 1 := by
        ...
        omega
      have hηr : η.getD (r : ℕ) 0 = 0 := by
        have hlen : η.length ≤ (r : ℕ) + 1 := by omega
        have : η.getD ((r : ℕ) + 1) 0 = 0 := List.getD_eq_default _ _ hlen
        omega
      simp only [frobVec, hirev, hηr, hi0'] at hpos
      omega
    ...
    have hjval : frobVec N η j = η.getD ((r : ℕ) + 1) 0 + ((i : ℕ) - 1) := by
      rw [frobVec, hjrev, hjv]
    have hival : frobVec N η i = η.getD (r : ℕ) 0 + (i : ℕ) := by
...
/-- **The Frobenius formula**: the number of standard Young tableaux of shape `η`,
multiplied by the product of the factorials of the first column hook lengths of `η`
(padded to `N ≥ length η` rows), is `|η| !` times the Vandermonde product of the first
column hook lengths. -/
theorem numStdTab_mul_prod_factorial (N : ℕ) :
    ∀ (n : ℕ) (η : List ℕ), IsPart η → η.length ≤ N → η.sum = n →
      (numStdTab η : ℤ) * ∏ i : Fin N, ((Nat.factorial (frobVec N η i) : ℕ) : ℤ)
        = ((Nat.factorial n : ℕ) : ℤ) * vdmProd (fun i => ((frobVec N η i : ℕ) : ℤ)) := by
  intro n
  induction n using Nat.strong_induction_on with
  | _ n ih =>
    intro η hη hlen hsum
    match n, hsum with
    | 0, hsum =>
      have hnil : η = [] := by
        have := hη.length_le_sum
        rw [hsum] at this
    ...
    | (m + 1), hsum =>
      classical
      set x : Fin N → ℤ := fun i => ((frobVec N η i : ℕ) : ℤ) with hx
      set T : Fin N → ℤ := fun i => x i * vdmProd (Function.update x i (x i - 1)) with hT
      have hbranch : (numStdTab η : ℤ) * ∏ i : Fin N, ((Nat.factorial (frobVec N η i) : ℕ) : ℤ)
          = ∑ r : Fin N, ((Nat.factorial m : ℕ) : ℤ) * T r.rev := by
        rw [numStdTab_branching hη hsum hlen, ← Fin.sum_univ_eq_sum_range
          (fun r => if IsRemCorner η r then numStdTab (decrNth η r) else 0)]
        push_cast
        rw [Finset.sum_mul]
        refine Finset.sum_congr rfl fun r _ => ?_
        by_cases hc : IsRemCorner η (r : ℕ)
        · rw [ite_eq_left hc]
          have hν : IsPart (decrNth η (r : ℕ)) := isPart_decrNth hη hc
          have hνlen : (decrNth η (r : ℕ)).length ≤ N :=
            le_trans (included_decrNth η (r : ℕ)).length_le hlen
          have hνsum : (decrNth η (r : ℕ)).sum = m := by
            rw [sum_decrNth hη hc, hsum]
            omega
          have hIH := ih m (by omega) (decrNth η (r : ℕ)) hν hνlen hνsum
          have hfact := frobVec_prod_factorial_decrNth (r := r) hη hc
          have hcast : ((frobVec N η r.rev : ℕ) : ℤ)
              * ∏ p : Fin N, ((Nat.factorial (frobVec N (decrNth η (r : ℕ)) p) : ℕ) : ℤ)
              = ∏ p : Fin N, ((Nat.factorial (frobVec N η p) : ℕ) : ℤ) := by
            rw [← Nat.cast_prod, ← Nat.cast_prod, ← Nat.cast_mul, hfact]
          calc ((numStdTab (decrNth η (r : ℕ)) : ℕ) : ℤ)
                * ∏ p : Fin N, ((Nat.factorial (frobVec N η p) : ℕ) : ℤ)
              = ((frobVec N η r.rev : ℕ) : ℤ) * (((numStdTab (decrNth η (r : ℕ)) : ℕ) : ℤ)
                * ∏ p : Fin N, ((Nat.factorial (frobVec N (decrNth η (r : ℕ)) p) : ℕ) : ℤ)) := by
                rw [← hcast]; ring
            _ = ((frobVec N η r.rev : ℕ) : ℤ) * (((Nat.factorial m : ℕ) : ℤ)
                * vdmProd (fun p => ((frobVec N (decrNth η (r : ℕ)) p : ℕ) : ℤ))) := by
                rw [hIH]
            _ = ((Nat.factorial m : ℕ) : ℤ) * T r.rev := by
                rw [frobVec_decrNth hη hc, hT]
                simp only [hx]
                ring
        · rw [ite_eq_right hc, hT]
          simp only [zero_mul, hx]
          rw [frobVec_term_eq_zero_of_not_remCorner hη hlen hc, mul_zero]
