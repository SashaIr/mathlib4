...
lemma cast_numStdTab_partsList (n : ℕ) (lam : Nat.Partition n) :
    (numStdTab lam.partsList : ℚ) = (Nat.factorial n : ℚ) / (hookProd lam.partsList : ℚ) := by
  have hpart : IsPart lam.partsList := Nat.Partition.isPart_partsList lam
  have hpos : (0 : ℚ) < (hookProd lam.partsList : ℚ) := by
...
theorem factorial_eq_sum_sq_factorial_div_hookProd (n : ℕ) :
    Nat.factorial n =
      ∑ lam : Nat.Partition n, (Nat.factorial n / hookProd lam.partsList) ^ 2 := by
  have h : ∀ lam : Nat.Partition n,
      numStdTab lam.partsList = Nat.factorial n / hookProd lam.partsList := fun lam ↦ by
    rw [numStdTab_eq_factorial_div_hookProd (Nat.Partition.isPart_partsList lam),
      Nat.Partition.sum_partsList]
  calc Nat.factorial n = ∑ lam : Nat.Partition n, (numStdTab lam.partsList) ^ 2 :=
        (sum_sq_numStdTab n).symm
    _ = _ := Finset.sum_congr rfl fun lam _ ↦ by rw [h lam]
...
theorem inv_factorial_eq_sum_inv_sq_hookProd (n : ℕ) :
    (1 : ℚ) / (Nat.factorial n : ℚ) =
      ∑ lam : Nat.Partition n, 1 / (hookProd lam.partsList : ℚ) ^ 2 := by
  have hfac : (Nat.factorial n : ℚ) ≠ 0 := Nat.cast_ne_zero.2 (Nat.factorial_ne_zero n)
  have h : ∑ lam : Nat.Partition n, (numStdTab lam.partsList : ℚ) ^ 2
      = (Nat.factorial n : ℚ) := by
    exact_mod_cast congrArg (Nat.cast (R := ℚ)) (sum_sq_numStdTab n)
  have h2 : (∑ lam : Nat.Partition n, 1 / (hookProd lam.partsList : ℚ) ^ 2)
      * (Nat.factorial n : ℚ) ^ 2 = (Nat.factorial n : ℚ) := by
    rw [Finset.sum_mul]
    calc ∑ lam : Nat.Partition n,
          1 / (hookProd lam.partsList : ℚ) ^ 2 * (Nat.factorial n : ℚ) ^ 2
        = ∑ lam : Nat.Partition n, (numStdTab lam.partsList : ℚ) ^ 2 :=
          Finset.sum_congr rfl fun lam _ ↦ by
            rw [cast_numStdTab_partsList n lam, div_pow]
            ring
