...
`p ++ μ ++ s` yields a colouring of `p ++ mv ++ s`, provided the four families of
comparisons hold. -/
lemma isGreeneDecCol_mixCol {k : ℕ} {p s μ mv : List T} {c wc : ℕ → Option ℕ} {τ : ℕ → ℕ}
    (hμ : μ.length = 3) (hmv : mv.length = 3)
    (hc : IsGreeneDecCol (p ++ μ ++ s) k c)
  ...
  · intro i j x hij hj hci hcj
    ...
    · rw [mixCol_lt hi] at hci
      ...
      · rw [mixCol_lt hjp] at hcj
        ...
        have hlt := hc.gt_of_colour hij (show j < (p ++ μ ++ s).length by omega) hci hcj
    ...
    · by_cases hi3 : i < p.length + 3
      ...
      · rw [mixCol_gt (by omega)] at hci
        ...
        have huj : (p ++ μ ++ s)[j]'(by omega) = s[j - p.length - 3]'(by omega) :=
          getElem_mid_right' p μ s (by omega) (by omega) _
        have hlt := hc.gt_of_colour hij (show j < (p ++ μ ++ s).length by omega) hy hy'
        exact (hvj.trans_lt (huj.symm.trans_lt (hlt.trans_eq hui))).trans_eq hvi.symm
...
lemma greeneSize_le_greeneCol_mix {k : ℕ} {p s μ mv : List T} {c wc : ℕ → Option ℕ} {τ : ℕ → ℕ}
    (hμ : μ.length = 3) (hmv : mv.length = 3)
    (hc : IsGreeneDecCol (p ++ μ ++ s) k c)
    (hτinj : Function.Injective τ) (hτk : ∀ x, x < k → τ x < k)
    (hwck : ∀ j x, j < 3 → wc j = some x → x < k)
    ...
    greeneSize (p ++ μ ++ s) c ≤ greeneCol (p ++ mv ++ s) k := by
  rw [← greeneSize_mixCol (μ := μ) (mv := mv) (τ := τ) hμ hmv hcount]
  exact le_greeneCol (isGreeneDecCol_mixCol hμ hmv hc hτinj hτk hwck h11 h12 h23 h13)
