...
theorem cauchy_hProd_monomialSym (m k n : ℕ) (R : Type*) [CommRing R] :
  ...
  rw [cauchy]
  ext d
  have hL : (∑ a ∈ Finset.Nat.antidiagonalTuple k n,
        C (∏ j : Fin k, hsymm (Fin m) R (a j))
          * ∏ j : Fin k, (X j : MvPolynomial (Fin k) (MvPolynomial (Fin m) R)) ^ a j).coeff d
  ...
  have hR : (∑ ν : PartLengthLe n k,
        C (hProd m R ν.1) * monomialSym k (MvPolynomial (Fin m) R) ν.1).coeff d
      = if ∑ j, d j = n then hProd m R (degShape d) else 0 := by
    rw [coeff_sum]
