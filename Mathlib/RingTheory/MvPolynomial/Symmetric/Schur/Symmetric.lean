...
theorem coeff_schurPoly_eq_kostkaNum (μ : List ℕ) (d : Fin m →₀ ℕ) :
    (schurPoly (Fin m) R μ).coeff d = (kostkaNum m μ (finContent m d) : R) := by
...
theorem coeff_schurPoly_mapDomain (μ : List ℕ) (d : Fin m →₀ ℕ) (e : Equiv.Perm (Fin m)) :
    (schurPoly (Fin m) R μ).coeff (Finsupp.mapDomain e d)
      = (schurPoly (Fin m) R μ).coeff d := by
  ...
  · rw [schurPoly_eq_zero_of_not_isPart hμ]
