...
* `MvPolynomial.LRtabCoeff Q₁ Q₂ nu` : the number of standard tableaux of shape `nu`
...
* `MvPolynomial.LRtabCoeff_eq_lrCoeff` : that number is the Littlewood–Richardson coefficient
  `c^nu_{shape Q₁, shape Q₂}` (Coq `LRtab_coeffP`).
...
/-- The number of standard tableaux of shape `nu` completing `Q₁` and `Q₂` into a
Littlewood–Richardson triple (Coq `LRtab_coeff`). -/
noncomputable def LRtabCoeff (Q₁ Q₂ : List (List ℕ)) (nu : List ℕ) : ℕ :=
  Nat.card {Q : LRsupport Q₁ Q₂ // shape Q.1 = nu}
...
/-- **The number of tableaux of shape `nu` in the Littlewood–Richardson support of `Q₁` and
`Q₂` is the Littlewood–Richardson coefficient `c^nu_{shape Q₁, shape Q₂}`** (Coq
`LRtab_coeffP`). -/
theorem LRtabCoeff_eq_lrCoeff (hQ₁ : IsStdTab Q₁) (hQ₂ : IsStdTab Q₂) {nu : List ℕ}
    (hnu : IsPart nu) (hsum : nu.sum = sizeTab Q₁ + sizeTab Q₂) :
    LRtabCoeff Q₁ Q₂ nu = lrCoeff (shape Q₁) (shape Q₂) nu := by
  classical
  set n := sizeTab Q₁ + sizeTab Q₂ with hn
  set g : LRsupport Q₁ Q₂ → PartIdx n n := fun Q => ⟨shape Q.1, shape_mem_partIdx Q⟩
  have hcard : ∀ nu : PartIdx n n,
      ∑ _Q : {Q : LRsupport Q₁ Q₂ // g Q = nu}, schurPoly (Fin n) ℤ nu.1
        = Fintype.card {Q : LRsupport Q₁ Q₂ // g Q = nu} • schurPoly (Fin n) ℤ nu.1 := by
    intro nu
    simp [Finset.card_univ]
  have hmul : ∑ nu : PartIdx n n,
      Fintype.card {Q : LRsupport Q₁ Q₂ // g Q = nu} • schurPoly (Fin n) ℤ nu.1
      = ∑ nu : PartIdx n n, lrCoeff (shape Q₁) (shape Q₂) nu.1 • schurPoly (Fin n) ℤ nu.1 := by
  ...
  have hnulen : nu.length ≤ n := by rw [← hsum]; exact hnu.length_le_sum
  set nuIdx : PartIdx n n := ⟨nu, hnu, hsum, hnulen⟩
  have hfun := congrFun (eq_of_sum_nsmul_schurPoly_eq hmul) nuIdx
  have hequiv : {Q : LRsupport Q₁ Q₂ // shape Q.1 = nu} ≃ {Q : LRsupport Q₁ Q₂ // g Q = nuIdx} :=
...
empty. -/
lemma LRtabCoeff_eq_zero {nu : List ℕ} (h : ¬ (IsPart nu ∧ nu.sum = sizeTab Q₁ + sizeTab Q₂)) :
    LRtabCoeff Q₁ Q₂ nu = 0 := by
...
theorem LRtabCoeff_eq_of_shape_eq (hQ₁ : IsStdTab Q₁) (hQ₂ : IsStdTab Q₂) (hT₁ : IsStdTab T₁)
    (hT₂ : IsStdTab T₂) (h₁ : shape Q₁ = shape T₁) (h₂ : shape Q₂ = shape T₂) (nu : List ℕ) :
    LRtabCoeff Q₁ Q₂ nu = LRtabCoeff T₁ T₂ nu := by
  have hs₁ : sizeTab Q₁ = sizeTab T₁ := congrArg List.sum h₁
  have hs₂ : sizeTab Q₂ = sizeTab T₂ := congrArg List.sum h₂
  by_cases h : IsPart nu ∧ nu.sum = sizeTab Q₁ + sizeTab Q₂
