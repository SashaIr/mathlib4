...
the words whose recording tableau is `Q`, that is, of the language `Young.language σ Q`.  Its
...
Combined with the free Littlewood–Richardson rule `Young.LRrule_language`, this gives the
...
* `MvPolynomial.schurPoly_eq_sum_language` : the commutative image of the free Schur function of
  `Q` is the Schur polynomial of the shape of `Q` (Coq `Schur_freeSchurE`).
* `MvPolynomial.polyLang_language` : the same statement in terms of the generating polynomial
  of a language.
* `MvPolynomial.catLang_language` : **the free Littlewood–Richardson rule** as an identity
...
theorem RS_bijOn_language {Q : List (List ℕ)} (hQ : IsStdTab Q) :
    Set.BijOn RS (language σ Q) {P : List (List σ) | IsTableau P ∧ shape P = shape Q} := by
  refine ⟨?_, ?_, ?_⟩
  · intro w hw
...
finite set. -/
instance finite_language [Finite σ] (Q : List (List ℕ)) : Finite (language σ Q) := by
  have hfin : Finite {w : List σ // w.length = sizeTab Q} :=
    inferInstanceAs (Finite (List.Vector σ (sizeTab Q)))
  refine Finite.of_injective
    (fun w : language σ Q => (⟨w.1, length_eq_sizeTab_of_mem_language w.2⟩ :
      {w : List σ // w.length = sizeTab Q})) ?_
  rintro ⟨u, hu⟩ ⟨v, hv⟩ h
  have : u = v := congrArg Subtype.val h
  exact Subtype.ext this

noncomputable instance fintypeLanguage [Finite σ] (Q : List (List ℕ)) : Fintype (language σ Q) :=
  Fintype.ofFinite _
...
noncomputable def languageEquivSSYT {Q : List (List ℕ)} (hQ : IsStdTab Q) :
    language σ Q ≃ SSYT σ (shape Q) :=
  (RS_bijOn_language hQ).equiv
...
theorem schurPoly_eq_sum_language [Fintype σ] {Q : List (List ℕ)} (hQ : IsStdTab Q) :
    schurPoly σ R (shape Q) = ∑ w : language σ Q, ((w.1.map (X : σ → MvPolynomial σ R)).prod) := by
  rw [schurPoly]
  refine (Fintype.sum_equiv (languageEquivSSYT (σ := σ) hQ) _ _ ?_).symm
  intro w
...
lemma isHomLang_language (Q : List (List ℕ)) : IsHomLang (sizeTab Q) (language σ Q) :=
  fun _ hw => length_eq_sizeTab_of_mem_language hw

/-- **The generating polynomial of the language of a standard tableau `Q` is the Schur
polynomial of the shape of `Q`** (Coq `Schur_freeSchurE`). -/
theorem polyLang_language [Fintype σ] {Q : List (List ℕ)} (hQ : IsStdTab Q) :
    polyLang R (language σ Q) = schurPoly σ R (shape Q) :=
  (schurPoly_eq_sum_language hQ).symm
...
lemma mem_language_take_drop (hQ₁ : IsStdTab Q₁) {Q : List (List ℕ)} (hLR : LRtriple Q₁ Q₂ Q)
    {w : List σ} (hw : w ∈ language σ Q) :
    w.take (sizeTab Q₁) ∈ language σ Q₁ ∧ w.drop (sizeTab Q₁) ∈ language σ Q₂ := by
  obtain ⟨u, v, rfl, hu, hv⟩ := (LRrule_language hQ₁ w).2 ⟨Q, hLR, hw⟩
  have hlen : u.length = sizeTab Q₁ := length_eq_sizeTab_of_mem_language hu
...
def languageProdEquivSigma (hQ₁ : IsStdTab Q₁) (hQ₂ : IsStdTab Q₂) :
    language σ Q₁ × language σ Q₂ ≃ Σ Q : LRsupport Q₁ Q₂, language σ Q.1 where
  toFun p :=
    ⟨⟨RSQ (p.1.1 ++ p.2.1),
      isStdTab_of_LRtriple hQ₁ hQ₂ (LRtriple_RSQ_append p.1.2 p.2.2),
      LRtriple_RSQ_append p.1.2 p.2.2⟩, ⟨p.1.1 ++ p.2.1, rfl⟩⟩
  invFun q :=
    (⟨q.2.1.take (sizeTab Q₁), (mem_language_take_drop hQ₁ q.1.2.2 q.2.2).1⟩,
     ⟨q.2.1.drop (sizeTab Q₁), (mem_language_take_drop hQ₁ q.1.2.2 q.2.2).2⟩)
  left_inv p := by
    have hlen : p.1.1.length = sizeTab Q₁ := length_eq_sizeTab_of_mem_language p.1.2
    ext1 <;> simp [← hlen]
  right_inv q := by
    obtain ⟨⟨Q, hQ⟩, ⟨w, hw⟩⟩ := q
    have hcat : w.take (sizeTab Q₁) ++ w.drop (sizeTab Q₁) = w := List.take_append_drop _ _
    have hQw : RSQ w = Q := mem_language.1 hw
...
theorem catLang_language (hQ₁ : IsStdTab Q₁) :
    catLang (language σ Q₁) (language σ Q₂) = {w | ∃ Q, LRtriple Q₁ Q₂ Q ∧ w ∈ language σ Q} := by
  ext w
  rw [mem_catLang]
  constructor
  · rintro ⟨u, hu, v, hv, rfl⟩
    exact (LRrule_language hQ₁ _).1 ⟨u, v, rfl, hu, hv⟩
  · intro h
    obtain ⟨u, v, rfl, hu, hv⟩ := (LRrule_language hQ₁ w).2 h
    exact ⟨u, hu, v, hv, rfl⟩
...
theorem schurPoly_mul_eq_sum_LRtriple [Fintype σ] (hQ₁ : IsStdTab Q₁) (hQ₂ : IsStdTab Q₂) :
  ...
  classical
  rw [schurPoly_eq_sum_language (R := R) hQ₁, schurPoly_eq_sum_language (R := R) hQ₂,
    Finset.sum_mul_sum]
  have hright : ∑ Q : LRsupport Q₁ Q₂, schurPoly σ R (shape Q.1)
      = ∑ q : Σ Q : LRsupport Q₁ Q₂, language σ Q.1,
          ((q.2.1.map (X : σ → MvPolynomial σ R)).prod) := by
    rw [← Finset.univ_sigma_univ, Finset.sum_sigma]
    exact Finset.sum_congr rfl fun Q _ => schurPoly_eq_sum_language (R := R) Q.2.1.1
  have hleft : ∑ i : language σ Q₁, ∑ j : language σ Q₂,
        ((i.1.map (X : σ → MvPolynomial σ R)).prod * (j.1.map (X : σ → MvPolynomial σ R)).prod)
      = ∑ p : language σ Q₁ × language σ Q₂,
          (((p.1.1 ++ p.2.1).map (X : σ → MvPolynomial σ R)).prod) := by
    rw [Fintype.sum_prod_type]
    exact Finset.sum_congr rfl fun i _ => Finset.sum_congr rfl fun j _ => by
      simp [List.map_append, List.prod_append]
  rw [hright, hleft]
  exact Fintype.sum_equiv (languageProdEquivSigma hQ₁ hQ₂) _ _ fun _ => rfl
