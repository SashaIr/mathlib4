...
lemma partdom_of_coeff_ne_zero [CommSemiring R] {ν μ : List ℕ} (hlen : ν.length ≤ m)
    (h : (schurPoly (Fin m) R μ).coeff (shapeContent m ν) ≠ 0) : Partdom ν μ := by
  by_contra hdom
...
theorem schurPoly_ne_zero [CommSemiring R] [Nontrivial R] {μ : List ℕ} (hμ : IsPart μ)
  ...
  rw [h] at hone
  simp only [AddMonoidAlgebra.coeff_zero, Finsupp.coe_zero, Pi.zero_apply] at hone
...
theorem linearIndependent_schurPoly (m n : ℕ) (R : Type*) [CommRing R] :
  ...
  have hsingle : ∀ ν ∈ s, ν ≠ ρ →
      (g ν • schurPoly (Fin m) R ν.1).coeff (shapeContent m ρ.1) = 0 := by
    intro ν hνs hνne
    rw [coeff_smul, smul_eq_mul]
    by_cases hνt : ν ∈ t
    · have hzero : (schurPoly (Fin m) R ν.1).coeff (shapeContent m ρ.1) = 0 := by
        by_contra hc
