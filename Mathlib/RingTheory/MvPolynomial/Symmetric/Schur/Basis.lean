...
theorem monomialSym_mem_span_schurPoly [CommRing R] (n : ℕ) {μ : List ℕ}
  ...
  | _ w ih =>
    ...
    have hexp : p = ∑ ν ∈ insert μ S, p.coeff (shapeContent m ν) • monomialSym m R ν := by
    ...
    have hkey : monomialSym m R μ
        = p - ∑ ν ∈ (insert μ S).erase μ,
            p.coeff (shapeContent m ν) • monomialSym m R ν :=
...
theorem schurPoly_eq_sum_kostkaNum_monomialSym [CommRing R] (μ : List ℕ) :
  ...
  have hcoeff : ∀ ν ∈ partFinsetLengthLe μ.sum m,
      p.coeff (shapeContent m ν) = (kostkaNum m μ (fun i => ν.getD i 0) : R) := by
    intro ν hν
  ...
  have hzero : ∀ ν ∈ partFinsetLengthLe μ.sum m, ν ∉ p.support.image degShape →
      p.coeff (shapeContent m ν) • monomialSym m R ν = 0 := by
    intro ν hν hnotin
    have : p.coeff (shapeContent m ν) = 0 := by
      by_contra hne
      exact hnotin (Finset.mem_image.2 ⟨shapeContent m ν, mem_support_iff.2 hne,
        degShape_shapeContent (mem_partFinsetLengthLe.1 hν).1 (mem_partFinsetLengthLe.1 hν).2.2⟩)
    rw [this, zero_smul]
  calc p = ∑ ν ∈ p.support.image degShape, p.coeff (shapeContent m ν) • monomialSym m R ν :=
        eq_sum_monomialSym hsym
    _ = ∑ ν ∈ partFinsetLengthLe μ.sum m, p.coeff (shapeContent m ν) • monomialSym m R ν :=
