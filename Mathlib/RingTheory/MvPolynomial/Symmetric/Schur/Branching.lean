...
lemma mapDomain_castSucc_apply_castSucc (u : Fin m →₀ ℕ) (i : Fin m) :
  ...
  Finsupp.mapDomain_apply_of_injective (Fin.castSucc_injective m) u i
...
lemma coeff_rename_castSucc_mul_pow (P : MvPolynomial (Fin m) R) (j : ℕ)
    (d : Fin (m + 1) →₀ ℕ) :
    (rename Fin.castSucc P * X (Fin.last m) ^ j).coeff d
      = if d (Fin.last m) = j then P.coeff (restrictLast d) else 0 := by
  ...
  | add P Q hP hQ =>
    rw [map_add, add_mul]
    simp only [AddMonoidAlgebra.coeff_add, Finsupp.coe_add, Pi.add_apply, hP, hQ]
    split_ifs
    · rfl
    · rw [add_zero]
  ...
  have hterm : ∀ k ∈ Finset.range (μ.sum + 1),
    ...
    · rw [ite_eq_left hlast, Nat.cast_sum]
      ...
      · rw [ite_eq_right hstrip, ite_eq_right hstrip]
        simp only [AddMonoidAlgebra.coeff_zero, Finsupp.coe_zero, Pi.zero_apply, Nat.cast_zero]
