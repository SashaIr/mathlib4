...
basis of Schur polynomials, `h_μ = ∑_mu K_{ν μ} s_ν`, where `K_{ν μ}` is the
...
* `MvPolynomial.hProd_eq_sum_kostka` : `h_μ = ∑_mu K_{ν μ} s_ν`.
...
/-- **The expansion of `h_μ` in the Schur polynomials**: `h_μ = ∑_mu K_{ν μ} s_ν`,
...
/-- `h_μ` is a symmetric homogeneous polynomial of degree `|μ|`. -/
lemma hProd_mem_symHomogeneousSubmodule [CommRing R] {n : ℕ} (μ : PartIdx n m) :
...
theorem hProd_eq_sum_partsFinset [CommRing R] (m : ℕ) {μ : List ℕ} (hμ : IsPart μ) :
    hProd m R μ
      = ∑ ν ∈ partsFinset μ.sum m, (kostka ν μ : R) • schurPoly (Fin m) R ν := by
  classical
  rw [hProd_eq_sum_kostka m hμ]
  refine (Finset.sum_subset ?_ ?_).symm
  · intro ν hν
    obtain ⟨h1, h2, -⟩ := mem_partsFinset.1 hν
...
/-- The expansion of `h_μ` in the Schur polynomials, indexed by `PartIdx n m`. -/
lemma hProd_eq_sum_partIdx [CommRing R] {n : ℕ} (μ : PartIdx n m) :
    hProd m R μ.1
      = ∑ ν : PartIdx n m, (kostka ν.1 μ.1 : R) • schurPoly (Fin m) R ν.1 := by
  classical
  rw [hProd_eq_sum_partsFinset m μ.2.1, μ.2.2.1, partsFinset,
...
theorem linearIndependent_hProd (m n : ℕ) (R : Type*) [CommRing R] :
    LinearIndependent R fun μ : PartIdx n m => hProd m R μ.1 := by
  classical
  rw [Fintype.linearIndependent_iff]
  intro g hg μ
  have hexp : ∑ ν : PartIdx n m, g ν • hProd m R ν.1
      = ∑ ρ : PartIdx n m,
          (∑ ν : PartIdx n m, g ν * (kostka ρ.1 ν.1 : R)) • schurPoly (Fin m) R ρ.1 := by
    simp only [hProd_eq_sum_partIdx, Finset.smul_sum, Finset.sum_smul, smul_smul]
    exact Finset.sum_comm
  have hcoef : ∀ ρ : PartIdx n m, ∑ ν : PartIdx n m, g ν * (kostka ρ.1 ν.1 : R) = 0 :=
...
theorem schurPoly_mem_span_hProd [CommRing R] (n : ℕ) {μ : List ℕ} (hμ : IsPart μ)
    ...
    schurPoly (Fin m) R μ
      ∈ Submodule.span R (Set.range fun ν : PartIdx n m => hProd m R ν.1) := by
  classical
  set W := Submodule.span R (Set.range fun ν : PartIdx n m => hProd m R ν.1) with hW
  ...
  | _ k ih =>
    ...
    have hexp := hProd_eq_sum_partsFinset (R := R) m hρ
    rw [hρsum] at hexp
    ...
    have hkey : schurPoly (Fin m) R ρ
        = hProd m R ρ - ∑ ν ∈ (partsFinset n m).erase ρ,
...
theorem span_hProd [CommRing R] (m n : ℕ) :
    Submodule.span R (Set.range fun μ : PartIdx n m => hProd m R μ.1)
...
of degree `n`. -/
noncomputable def hSub (m n : ℕ) (R : Type*) [CommRing R] (μ : PartIdx n m) :
    symHomogeneousSubmodule m n R :=
  ⟨hProd m R μ.1, hProd_mem_symHomogeneousSubmodule μ⟩

@[simp] lemma coe_hSub (m n : ℕ) (R : Type*) [CommRing R] (μ : PartIdx n m) :
...
lemma coe_hBasis (m n : ℕ) (R : Type*) [CommRing R] (μ : PartIdx n m) :
