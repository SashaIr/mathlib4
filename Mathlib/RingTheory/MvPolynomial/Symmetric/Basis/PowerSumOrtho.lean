...
lemma eq_sum_hallInner_smul_of_dual [CommRing R]
    ...
    f = ∑ μ : PartIdx n m, hallInner m n R f (v μ) • u μ := by
  classical
  ...
  have hfin : ∀ κ : PartIdx n m,
      ∑ μ : PartIdx n m,
          (schurBasis m n R).repr f κ * (B.transpose κ μ * A μ ρ)
        = (schurBasis m n R).repr f κ * (1 : Matrix (PartIdx n m) (PartIdx n m) R) κ ρ := by
    intro κ
...
@[simp] lemma basisOfHallInnerDual_apply [CommRing R]
    (u v : PartIdx n m → symHomogeneousSubmodule m n R)
    (h : ∀ μ ν, hallInner m n R (u μ) (v ν) = if μ = ν then 1 else 0)
    (μ : PartIdx n m) : basisOfHallInnerDual u v h μ = u μ := by
...
`⟨p_μ, p_μ⟩ = z_μ`. -/
theorem hallInner_pSub [CommRing R] [Algebra ℚ R] (hnm : n ≤ m) (μ ν : PartIdx n m) :
...
theorem eq_sum_hallInner_pSubInvZ_smul [CommRing R] [Algebra ℚ R] (hnm : n ≤ m)
    (f : symHomogeneousSubmodule m n R) :
    f = ∑ μ : PartIdx n m, hallInner m n R f (pSubInvZ m n R μ) • pSub m n R μ :=
