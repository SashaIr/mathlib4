...
theorem eq_sum_monomialSym_partFinsetLengthLe {n : ℕ} {p : MvPolynomial (Fin m) R}
    ...
    p = ∑ μ ∈ partFinsetLengthLe n m, p.coeff (shapeContent m μ) • monomialSym m R μ := by
  classical
  ...
  have hzero : ∀ μ ∈ partFinsetLengthLe n m, μ ∉ p.support.image degShape →
      p.coeff (shapeContent m μ) • monomialSym m R μ = 0 := by
    intro μ hμ hnotin
    have hc : p.coeff (shapeContent m μ) = 0 := by
      by_contra hne
    ...
    rw [hc, zero_smul]
  calc p = ∑ μ ∈ p.support.image degShape, p.coeff (shapeContent m μ) • monomialSym m R μ :=
        eq_sum_monomialSym hsym
    _ = ∑ μ ∈ partFinsetLengthLe n m, p.coeff (shapeContent m μ) • monomialSym m R μ :=
