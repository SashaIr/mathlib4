...
The cycle index formula `h_n = ∑_{μ ⊢ n} p_μ / z_μ` of
...
`∑_{μ ⊢ n} P_μ / z_μ` satisfy the same recursion, and they are the unique family
...
* `MvPolynomial.genCycleIndexSum` : the sum `∑_{μ ⊢ n} P_μ / z_μ`.
...
/-- The product `P_μ = P_{μ_1} ⋯ P_{μ_k}` attached to a list of parts. -/
noncomputable def genProd (P : ℕ → A) (μ : List ℕ) : A := (μ.map P).prod

@[simp] lemma genProd_nil (P : ℕ → A) : genProd P [] = 1 := rfl

lemma genProd_cons (P : ℕ → A) (r : ℕ) (μ : List ℕ) :
    genProd P (r :: μ) = P r * genProd P μ := by
  simp [genProd]

lemma genProd_of_perm (P : ℕ → A) {l l' : List ℕ} (h : l.Perm l') :
    genProd P l = genProd P l' := (h.map P).prod_eq

lemma genProd_insPart (P : ℕ → A) (μ : List ℕ) (r : ℕ) :
    genProd P (insPart μ r) = P r * genProd P μ := by
  rw [genProd_of_perm P (insPart_perm μ r), genProd_cons]

/-- The sum `∑_{μ ⊢ n} P_μ / z_μ` attached to a family `P`. -/
noncomputable def genCycleIndexSum [Algebra ℚ A] (P : ℕ → A) (n : ℕ) : A :=
  ∑ μ ∈ partFinset n, ((zcard μ : ℚ))⁻¹ • genProd P μ
...
lemma sum_mul_genCycleIndexSum [Algebra ℚ A] (P : ℕ → A) (n : ℕ) :
  ...
  have hL : ∑ r ∈ Finset.Icc 1 n, P r * genCycleIndexSum P (n - r)
    ...
    rw [← Finset.sum_sigma' (Finset.Icc 1 n) (fun r => partFinset (n - r))
      (fun r ν => ((zcard ν : ℚ))⁻¹ • genProd P (insPart ν r))]
    refine Finset.sum_congr rfl fun r _ => ?_
    rw [genCycleIndexSum, Finset.mul_sum]
    exact Finset.sum_congr rfl fun ν _ => by rw [genProd_insPart, mul_smul_comm]
  have hR : (n : ℚ) • genCycleIndexSum P n
    ...
    rw [← Finset.sum_sigma' (partFinset n) (fun μ => μ.toFinset)
      (fun μ r => ((zcard (dropPart μ r) : ℚ))⁻¹ • genProd P μ),
...
`n · F n = ∑_{r=1}^n P_r · F (n-r)` is the family of the sums `∑_{μ ⊢ n} P_μ / z_μ`. -/
